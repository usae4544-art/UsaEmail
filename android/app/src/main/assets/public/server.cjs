...
Respond authentically and warmly like a real, deeply invested girlfriend. Keep responses short, engaging, emotional, and highly context-aware. NO EMOJIS. ONLY HINGLISH. NEVER ENGLISH.`;
      ...
      if (triggerVideo || triggerPhoto) {
        ...
        const generateImagePrompt = async () => {
          try {
            const ai = new import_genai.GoogleGenAI({ apiKey: keysToUse[0] });
      ...
      const fetchGroqFallback = async (groqRetries = 0) => {
      ...
      const fetchGeminiResponse = async (retries = 0) => {
        const currentKey = keysToUse[retries % keysToUse.length];
        const ai = new import_genai.GoogleGenAI({ apiKey: currentKey });
        try {
  ...
  app.post("/api/translate", async (req, res) => {
    ...
    try {
      const ai = new import_genai.GoogleGenAI({ apiKey: getCurrentApiKey() });
      const response = await ai.models.generateContent({
...
Text to translate: ${text}`
          ...
          config: {
            ...
            speechConfig: {
              voiceConfig: {
                ...
                // Kore is a softer, more romantic female voice
              }
            },
            safetySettings: [
              { category: import_genai.HarmCategory.HARM_CATEGORY_HARASSMENT, threshold: import_genai.HarmBlockThreshold.BLOCK_NONE },
  ...
  function getSmartGirlfriendReply(userMsg, affectionLevel) {
